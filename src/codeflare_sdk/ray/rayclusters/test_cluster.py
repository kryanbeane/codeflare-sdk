...
def test_cluster_apply_down(mocker):
    ...
    mocker.patch(
        "codeflare_sdk.ray.rayclusters.raycluster.RayCluster._throw_for_no_raycluster"
    )
    mocker.patch(
        "codeflare_sdk.ray.rayclusters.raycluster.RayCluster._build_standalone_ray_cluster",
        return_value="./tests/test_cluster_yamls/ray/default-ray-cluster.yaml",
    )
    # Mock DynamicClient to prevent HTTP requests during initialization
    mock_dynamic_client = mocker.Mock()
    mock_resources = mocker.Mock()
    mock_api_instance = mocker.Mock()
    mock_resources.get.return_value = mock_api_instance
    mock_dynamic_client.resources = mock_resources
    mocker.patch(
        "codeflare_sdk.ray.rayclusters.raycluster.DynamicClient",
...
def test_cluster_apply_scale_up_scale_down(mocker):
    ...
    cluster = create_cluster(initial_num_workers)
    ...
    cluster = create_cluster(scaled_up_num_workers)
    patch_cluster_with_dynamic_client(mocker, cluster, mock_dynamic_client)
    cluster.apply()
...
def test_cluster_apply_with_file(mocker):
    ...
    # Step 1: Create cluster with initial workers
    cluster = create_cluster(1, write_to_file=True)
    patch_cluster_with_dynamic_client(mocker, cluster, mock_dynamic_client)
    mocker.patch(
        "kubernetes.client.CustomObjectsApi.list_namespaced_custom_object",
        return_value=get_obj_none("ray.io", "v1", "ns", "rayclusters"),
...
def test_cluster_apply_basic(mocker):
    ...
    # Create a cluster configuration
    cluster = create_cluster(1, write_to_file=False)
    patch_cluster_with_dynamic_client(mocker, cluster, mock_dynamic_client)
...
def test_cluster_uris(mocker):
    ...
    mocker.patch("kubernetes.config.load_kube_config", return_value="ignore")
    mocker.patch(
        "codeflare_sdk.ray.rayclusters.raycluster.RayCluster._get_dashboard_url_from_httproute",
        return_value=None,
    )
    mocker.patch(
        "kubernetes.client.CustomObjectsApi.list_namespaced_custom_object",
    ...
    mocker.patch(
        "kubernetes.client.NetworkingV1Api.list_namespaced_ingress",
        return_value=ingress_retrieval(cluster_name="unit-test-cluster"),
    )
    ...
    mocker.patch(
        "codeflare_sdk.ray.rayclusters.raycluster.RayCluster._is_openshift_cluster",
        return_value=True,
    )
    mocker.patch(
        "kubernetes.client.CustomObjectsApi.list_namespaced_custom_object",
...
def test_local_client_url(mocker):
    ...
    assert (
        cluster.cluster_uri()
        == "ray://unit-test-cluster-localinter-head-svc.ns.svc:10001"
    )
...
def test_wait_ready(mocker, capsys):
    from codeflare_sdk.ray.rayclusters.status import CodeFlareClusterStatus

    mocker.patch("kubernetes.client.ApisApi.get_api_versions")
    mocker.patch(
        "kubernetes.client.NetworkingV1Api.list_namespaced_ingress",
        # Use the same cluster name as the RayCluster under test to avoid
        # "Dashboard not available" error strings during wait_ready().
        return_value=ingress_retrieval(cluster_name="test"),
    )
    mocker.patch("kubernetes.config.load_kube_config", return_value="ignore")
    mocker.patch(
    ...
    mocker.patch(
        "codeflare_sdk.ray.rayclusters.raycluster.RayCluster.status",
...
def test_list_queue_rayclusters(mocker, capsys):
    ...
    assert RayCluster._is_openshift_cluster() == True
    mocker.patch(
        "kubernetes.client.CustomObjectsApi.list_namespaced_custom_object",
        return_value=get_obj_none("ray.io", "v1", "ns", "rayclusters"),
    )
...
def test_list_clusters(mocker, capsys):
    from codeflare_sdk.ray.rayclusters.raycluster import list_all_clusters
...
def test_map_to_ray_cluster(mocker):
    from codeflare_sdk.ray.rayclusters.raycluster import _map_to_ray_cluster
...
def test_cluster_apply_attribute_error_handling(mocker):
    ...
    # Mock DynamicClient to raise AttributeError during apply(), which should be
    # converted into a RuntimeError by the RayCluster.apply() error handling.
    def raise_attribute_error(_client):
...
def test_cluster_namespace_handling(mocker, capsys):
    ...
    mocker.patch(
        "codeflare_sdk.ray.rayclusters.raycluster.get_current_namespace",
        return_value=None,
    )
    ...
    # Explicitly trigger namespace resolution to capture the warning.
    cluster._ensure_namespace()
    captured = capsys.readouterr()
...
def test_component_resources_with_write_to_file(mocker):
    ...
    # Mock auth helpers and file writing to keep the test focused on the
    # write_to_file code path in _build_standalone_ray_cluster().
    ...
    mock_write = mocker.patch.object(
        cluster, "_write_to_file", return_value="test-cluster-component.yaml"
    )

    cluster._resource_yaml = cluster._build_standalone_ray_cluster()
    assert cluster._resource_yaml == "test-cluster-component.yaml"
    mock_write.assert_called_once()


def test_get_cluster_status_functions(mocker):
    """Test _ray_cluster_status method on RayCluster"""
    mocker.patch("kubernetes.config.load_kube_config", return_value="ignore")
    ...
    # Create a cluster instance to call the method on
    cluster = RayCluster(name="test-cluster", namespace="ns")
    ...
    result = cluster._ray_cluster_status("non-existent-cluster", "ns")
...
def test_get_dashboard_url_from_httproute(mocker):
    ...
    """
    # _get_dashboard_url_from_httproute is now a static method on RayCluster
    _get_dashboard_url_from_httproute = RayCluster._get_dashboard_url_from_httproute
