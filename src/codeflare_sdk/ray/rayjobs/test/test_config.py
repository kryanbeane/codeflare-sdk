"""
Tests for RayCluster accelerator_configs behavior.
"""

import pytest
from codeflare_sdk.ray.rayclusters.raycluster import DEFAULT_ACCELERATORS
...
def test_accelerator_configs_can_be_overridden():
    """Test that users can override default accelerator_configs with overwrite flag"""
    custom_configs = {
    ...
    # Should have overwritten the default nvidia.com/gpu mapping
    assert config.accelerator_configs["nvidia.com/gpu"] == "CUSTOM_GPU"
    # Should still have other defaults merged in
    assert "intel.com/gpu" in config.accelerator_configs
    assert "google.com/tpu" in config.accelerator_configs
...
def test_accelerator_configs_can_extend_defaults():
    ...
    config = RayCluster(accelerator_configs={"custom.com/accelerator": "CUSTOM_ACCEL"})

    # Should have all defaults plus custom (merged automatically)
    assert "nvidia.com/gpu" in config.accelerator_configs
    assert "intel.com/gpu" in config.accelerator_configs
...
def test_accelerator_configs_override_without_flag_raises_error():
    """Test that overriding default accelerator configs without flag raises ValueError"""
    with pytest.raises(
        ValueError, match="set overwrite_default_accelerator_configs to True"
    ):
        RayCluster(accelerator_configs={"nvidia.com/gpu": "CUSTOM_GPU"})
...
def test_managed_cluster_config_uses_update_image_for_worker(mocker):
    ...
    # Mock update_image where it's used, not where it's defined
    mock_update_image = mocker.patch(
        "codeflare_sdk.ray.rayclusters.raycluster.update_image",
        return_value="mocked-image:latest",
    )
