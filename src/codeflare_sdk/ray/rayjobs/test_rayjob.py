...
def test_rayjob_stop_kueue_managed(auto_mock_setup, caplog, capsys):
    """Test RayJob stop operation for Kueue-managed jobs."""
    mock_api_instance = auto_mock_setup["rayjob_api"]

    mock_api_instance.get_job.return_value = {
        "metadata": {
            "name": "test-rayjob",
            "labels": {"kueue.x-k8s.io/queue-name": "test-queue"},
        },
        "spec": {"suspend": False},
    }
    ...
    assert "is managed by Kueue" in caplog.text
    assert "cannot be suspended/stopped" in caplog.text

    captured = capsys.readouterr()
    assert "Cannot stop Kueue-managed RayJob" in captured.out
    assert "test-queue" in captured.out
    assert "job.delete()" in captured.out

    mock_api_instance.suspend_job.assert_not_called()
